 ...
 * `<prefix>-<series>-<NNNN>`.
 *
 * The same shape the counter emits: `CV-2026-0001` and `SMK-PPD-0001` differ
 * only in what occupies the prefix and series slots. There is one identifier
 * format in this system, not a manual one and an automatic one — the counter
 * happens to emit a `CV` prefix, but the grammar admits any studio's.
 *
 ...
 * a URL path segment.
 */
const ACCESSION_ID = /^([A-Z0-9]{2,8})-([A-Z0-9]{2,8})-\d{4}$/
...
export const ACCESSION_ID_HINT =
  "CV-STD-0001 — two segments of two to eight uppercase letters or digits, then four digits."
